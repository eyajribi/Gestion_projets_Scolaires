...
class ProfileRepository @Inject constructor(
    ...
    suspend fun updateUserProfile(
        ...
        return try {
            ...
            val response = apiService.updateEtudiant(token, userId, etudiant)
            if (response.isSuccessful && response.body() != null) {
                Result.success(
                    com.example.scolabstudentapp.models.User(
                        id = response.body()!!.id ?: userId,
                        nom = response.body()!!.nom,
                        prenom = response.body()!!.prenom,
                        email = response.body()!!.email,
                        numTel = response.body()!!.numTel,
                        urlPhotoProfil = response.body()!!.urlPhotoProfil,
                        nomFac = response.body()!!.nomFac,
                        nomDep = response.body()!!.nomDep,
                        role = "ETUDIANT",
                        estActif = response.body()!!.estActif ?: true,
                        emailVerifie = false,
                        isLoggedIn = true
                    )
                )
